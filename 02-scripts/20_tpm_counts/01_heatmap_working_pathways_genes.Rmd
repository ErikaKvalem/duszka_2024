...
comparison=c("CR.Ctrl", "ad.lib.Ctrl")
...
DF <- merge(DF, df_separate, by.x = "gene_name", by.y = "geneID")
...
comparison <- unique(filtered_allSampleAnno$sample_type)
p_values <- matrix(nrow = nrow(data_norm), ncol = length(comparison))

for (i in 1:nrow(data_norm)) {
  for (j in 1:length(comparison)) {
    group1 <- data_norm[i, filtered_allSampleAnno$sample_type == comparison[j]]
    group2 <- data_norm[i, filtered_allSampleAnno$sample_type != comparison[j]]
    p_values[i, j] <- wilcox.test(group1, group2, correct = TRUE)$p.value
  }
}
...
annotation_data = subset(annotation_data, select = -c(-V2) )
colnames(annotation_data) <- paste0(comparison[1],"_vs_",comparison[2])

# Convert p-values to a color scale
annotation_colors <- colorRamp2(c(0, 0.05, 1), c("green", "yellow", "red"))

# Create a heatmap for p-values
annotation_heatmap <- Heatmap(annotation_data, name = "p-values",
                              col = annotation_colors,
                              cell_fun = function(j, i, x, y, width, height, fill) {
                                grid.text(significance[i, j], x, y)
                              },
                              show_row_names = FALSE, show_column_names = TRUE,
                              show_heatmap_legend = TRUE)

main_heatmap <- Heatmap(data_norm, name = "z-score",
