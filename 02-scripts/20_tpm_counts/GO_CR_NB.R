...
bplapply(names(ora_tests), function(ora_name) {
  ...
  if (!is.null(ora_res)) {
    ...
    if (min(res_tab$p.adjust) < 0.05 & length(unique(res_tab$geneID)) > 1) {
      ...
      #p = barplot(ora_res, showCategory=30)# + scale_y_discrete(labels=function(x) str_wrap(x, width=30))
      save_plot(file.path(results_dir, paste0(prefix, "_ORA_", ora_name, "_dotplot")), p, width = 15, height = 10)
      #save_plot(file.path(results_dir, paste0(prefix, "_ORA_", ora_name, "_barplot")), p, width = 15, height = 10)
  ...
  bplapply(names(gsea_tests), function(gsea_name) {
    ...
    if (!is.null(gsea_res)) {
      ...
      if (min(res_tab$p.adjust) < 0.05) {
        ...
        #p = barplot(gsea_res, showCategory=15, split=".sign") #+ facet_grid(.~.sign) + scale_y_discrete(labels=function(x) str_wrap(x, width=30))
        
        save_plot(file.path(results_dir, paste0(prefix, "_GSEA_", gsea_name, "_dotplot")), p, width = 20, height = 15)
        #save_plot(file.path(results_dir, paste0(prefix, "_GSEA_", gsea_name, "_barplot")), p, width = 20, height = 15)
